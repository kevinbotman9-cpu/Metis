...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| The loop read as one decomposition, break included | BUILT | YES — `/performance` | NO | Taxonomy 14.2 `TABLE-STAKES`. [W-060](BACKLOG.md), [G-049](gaps.md). `docs/METIS_CONSOLE_SPEC.md` §4.7 Cascade, and the first screen on it: decisions → offered → deliverable → seen → acted, five stages that nest. The break at `deliverable` — 2,687 of 3,426 offered decisions on a channel nothing sends — is drawn in the block colour with its count stated on the stage, and every rate below it names the channel it describes. `apps/console/components/cascade-rail.tsx`, `apps/console/components/loop-flow.tsx`; `apps/console/tests/e2e/performance-cascade.spec.ts` is `@screen-only` and each of its six checks was verified to bite |
