...
| A refusal that cannot name its pack, its freshness, or what the customer was told | ABSENT | NO | NO | Taxonomy 14.4 `DIFFERENTIATING`. [W-066](BACKLOG.md), [G-055](gaps.md), [G-056](gaps.md), [G-057](gaps.md). A `TargetingPolicy` has no package, so a refusal cannot be attributed to the regulatory pack that supplied the rule. A `SourceCall` records `ms` and `cacheHit` and no timestamp, so a value read from cache cannot be dated — a decision can say the consent registry answered in 12ms and not when the flag it returned was true. And nothing anywhere records customer-facing refusal wording: every other part of "why was this not offered to me" is recorded to the field and the rule, and the half the customer experienced is not recorded at all. `/decisions/[id]` renders all three as explicit absences with their reason |
