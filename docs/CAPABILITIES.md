...
| Six of sixteen user-editable entities actually declared | PARTIAL | YES — `/objectives`, `/placements`, `/offers/[id]` | NO | `registry/index.ts` holds `Objective`, `Category`, `Placement`, `Offer`, `Creative` and `TenantSettings`; `USER_EDITABLE_ENTITIES` lists sixteen; `PENDING` records why each of the other ten is absent, per entity, with a reason. Rule 8 is enforced for **33%** of the entities it names, up from 14% on 2026-09-09. A test fails if an entity is in neither list, so the mechanism tracking the debt is better built than the mechanism it tracks |
