...
| Seeing which offers cannot be delivered, by channel | BUILT | YES — `/creatives?view=coverage` | NO | Taxonomy 5.7 `TABLE-STAKES`. One row per active offer against one column per channel served, in three states. Of 202 active offers, 38 have nothing live on any channel served and none has content on all five. Built as a view on `/creatives` rather than a route of its own: the same dataset asked a second question, with the answer in the URL so it can be linked. `apps/console/tests/e2e/creative-coverage.spec.ts` |
