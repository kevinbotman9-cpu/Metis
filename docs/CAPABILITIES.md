...
| The ranking formula's weights are declared, not hand-built | BUILT | YES — `/arbitration` | **YES** | Taxonomy 16.1 `FRONTIER`. `packages/ui-metadata/src/registry/arbitration-config.ts`, drawn inline by the generic renderer beside the formula it previews. Tests `the ArbitrationConfig descriptor, exercised`, `publishing arbitration weights survives a reload and is audited`. Edited only: a tenant has one, and one without it has no ranking function |
