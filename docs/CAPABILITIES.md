...
Registry            75 passed  - one behaviour suite, run against memory and a
                                 real PostgreSQL
Catalogue           63 passed  - taxonomy, offers, creatives, policies, boosts,
                                 connectors, placements, the profile schema,
                                 experiments and the ranking function; one
                                 suite over memory and a real PostgreSQL
...
| `offer` / `action` split | PARTIAL | NO | NO | `ENGINE-ONLY` in the survey's terms (the map above): works, and nothing can author an action. [ADR-019](adr/ADR-019-an-action-is-an-offer-made-decidable.md) §1–7. The catalogue snapshot carries `actions`, generated one per offer under the offer's key (`generateActions`, `packages/core/src/domain.ts`); both engines build candidates from them, joined to their offer, and refuse an action naming an absent offer or a key declared twice (`packages/runtime/src/deterministic/engine.ts`, `engines/kotlin/engine/src/main/kotlin/com/metis/engine/Engine.kt`). `PolicyScope` gains `action`: a boost or cap on an action is more specific than one on its offer, an offer's cap covers all its actions, and the ledger counts an action-scoped cap by action key (`packages/ledger/src/contacts.ts`, both stores). Held by five decision-corpus cases and two refusals in both engines (`decision-conformance.test.ts`, `DecisionConformanceTest`), `counts the contacts about one action, not its offer’s other action` (`packages/ledger/tests/suite.ts`, memory and PostgreSQL) and `reads a cap scoped to an action as the contacts about that action alone` (`contacts.test.ts`). **Limit:** actions are generated, not stored or editable — no Actions screen, no creative editor off the action, and a creative's key has not moved from offer to action (ADR-019 §5), by the product owner's decision of 2026-09-18 |
...
| Seeing what the engines are held to, from the console | PARTIAL | YES — `/` (the architect's Overview) | NO | `getConformance` is **proposed** and served by the console's development API, not a plane — [G-109](gaps.md), [W-078](BACKLOG.md). It counts the three committed corpora from their files and names the check that holds each engine to them: the `corpus` gate for TypeScript, `kotlin-conformance` in CI for Kotlin. It does not report whether they passed, because nothing it can run would know; `apps/console/tests/e2e/overview.spec.ts`, `names what holds each engine to the corpora, and never claims the checks passed` |
| The elimination funnel as the navigation | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 14.4 `DIFFERENTIATING`. [W-065](BACKLOG.md), [§4.7 Cascade](METIS_CONSOLE_SPEC.md). The design north star, rebuilt: the trace was a vertical list of nodes in a card, which showed the order and hid the shape — a reader could see seven nodes ran and not that 22 candidates became one. The rail is the funnel and is derived from the trace's own nodes, so a flow with one filter shows one, not three empty tiers; the middle pane groups removals by `ruleId`, which names something a person can change, rather than by the reason code a whole tier shares; the right pane holds the rule, its conditions, the connector that supplied the field, and the three things the platform does not record, each stated with its gap id. `apps/console/components/trace-cascade.ts`; `tests/unit/trace-cascade.test.ts` (11) holds the arithmetic — candidates are conserved at every stage — and `tests/e2e/trace-cascade.spec.ts` (9, `@screen-only`) holds the screen, including that the rail is operable from the keyboard alone |
| A refusal that names the pack behind its rule, and when the value it read was computed | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 14.4 `DIFFERENTIATING`, in part. [G-055](gaps.md), [G-056](gaps.md), [W-066](BACKLOG.md). The compiler resolves each policy a flow references to the pack that supplied it and pins it in the artifact — `packages/compiler/src/decision-flow/compile.ts`, test `records which pack supplied each rule the flow references`. `SourceCall` carries `fetchedAt` and `observedAt` — `packages/runtime/src/integration/resolve.ts`, tests `when a source value was computed`, including one that fails if a cache hit borrows the read time. The screen shows both: `apps/console/tests/unit/trace-evidence.test.tsx` (3), `names it, its version and its id`, and the two absences the pane distinguishes — a rule no pack claims, and an artifact that has not loaded. That check was an e2e walking `/decisions` for an affordability refusal until 2026-09-12; it held only because `telco-uk` installed two regulatory packs, so it is now a component test on its own fixture ([G-096](gaps.md) is the class). Neither field is hashed, so no chain hash moved. Per-rule source attribution still does not resolve in this tenant — [G-069](gaps.md) |
| A refusal that can say what the customer was told | ABSENT | NO | NO | Taxonomy 14.4 `DIFFERENTIATING`. [G-057](gaps.md), assessed 2026-09-11 and deliberately not built: where the wording lives, who authors it and whether the customer was told anything at all are product and legal decisions, and there is no message catalogue to hold customer-facing strings. `/decisions/[id]` renders it as an explicit absence |
| A node that names which question it answers | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 14.4. [G-058](gaps.md), [W-066](BACKLOG.md). The compiler derives the tier from the kinds of the policies a node declares and writes it onto the compiled node — `packages/compiler/src/decision-flow/compile.ts`, tests `names the tier from the policies a node declares, not from its id`. The rail reads it instead of matching node ids — `apps/console/tests/unit/trace-cascade.test.ts`. It names what a node declares, not everything it enforces: consent and frequency are applied at every constraint node, and consent by the platform's own `__consent` step where no constraint node comes before ranking — [G-070](gaps.md), [G-015](gaps.md) |
| Seeing where candidates fall out of decisions, by stage and by rule | PARTIAL | YES — `/targeting-policies?view=funnel` | NO | `getPolicyFunnel` is **proposed** and served by the console's development API, not a plane — [G-107](gaps.md), [W-076](BACKLOG.md). `buildPolicyFunnel` in `packages/ledger/src/policy-funnel.ts` stages removals by reason code in the order a decision meets them, so the stages nest whatever order a flow ran its nodes in; `packages/ledger/tests/policy-funnel.test.ts` (16). The seeded corpus is read from the ledger, whose records carry their eliminations whole — the `removals` column of the committed index was deleted with it ([ADR-018](adr/ADR-018-the-seeded-corpus-becomes-ledger-rows.md) §6); `apps/console/tests/unit/seeded-ledger.test.ts` holds the ledger equal to the generator and asserts every seeded decision's removals and winner account for its candidates. Whether a stage is asked comes from the compiled nodes of the flows in range, so a stage no flow asks reads as not asked rather than zero. The screen: `apps/console/tests/e2e/policy-funnel.spec.ts` `@screen-only`, asserting the funnel's structure rather than the corpus's figures, and two scans in `accessibility.spec.ts` |
| Stable reason codes, per candidate | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 14.4 `DIFFERENTIATING`. 10 codes in `REASON_CODES`, `packages/runtime/src/deterministic/types.ts`; `decision-conformance.test.ts` asserts the corpus exercises **every** one and `trace-cascade.test.ts` asserts the screen explains every one, both reading the closed set rather than a hand-written list. `NOT_ACTIVE`, `OUT_OF_VALIDITY_WINDOW`, `ELIGIBILITY_FAILED`, `RELEVANCE_FAILED`, `SUITABILITY_FAILED`, `CONSENT_WITHHELD`, `CONTACT_HISTORY_UNAVAILABLE` (ADR-021), `FREQUENCY_CAP_BREACHED`, `COOLDOWN_ACTIVE`, `NOT_RANKED` |
...
| Placement as a configured object | BUILT | YES — `/placements` | **YES** | Taxonomy 11.10 `DIFFERENTIATING`. `Placement` carries the slot count and the flow that answers it, so a website names a slot rather than a flow. **Not** part of the catalogue the engine hashes: a placement governs delivery, not the decision. The consequence is stated in `gaps.md` — a slate is reproducible from its decision *plus* the placement that composed it. Persisted by `packages/catalogue` since 2026-09-13 — `catalogue_placements`, one key per tenant, tests `refuses two placements sharing a key` over memory and PostgreSQL — and carried by the export; since 2026-09-14 the console reads and writes placements through it |
...
| Timing captured per decision | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 17.3 `FRONTIER` in part. `packages/runtime/tests/*` test `hot path cost`; the trace carries `totalMs`, measured when it executes. Search results and the decision index do not ([G-052](gaps.md)): the only value they ever held was the fixture generator's, frozen, so no screen shows latency over many decisions ([G-061](gaps.md)). Per-node attribution of that time is not surfaced |

## §7 — Intelligence

| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| Model gateway | OUT OF SCOPE | NO | NO | Taxonomy 8.7 `DIFFERENTIATING`. Gate 2. Nothing calls a model: every propensity is the seeded function behind `resolveScores`. The registry half of this row is the next one down. [W-029](BACKLOG.md) |
| Model versions in the registry, versioned like flows | BUILT | YES — `/models` | YES | Taxonomy 8.14. [ADR-009](adr/ADR-009-the-model-plane.md) §4, step two. `packages/registry/src/models.ts`; a version declares its kind, its features as data-model paths, a declared p95, an owner, a weights hash and the last date of its training data. Identical content republished is a no-op and different content under a published version is refused: `packages/registry/tests/models.ts` › `refuses different content under a published version, and keeps the original`, against memory and PostgreSQL, where `registry_models` is append-only by trigger. Published by clicking: `apps/console/tests/e2e/model-publishing.spec.ts` › `a published version cannot change under the same number, and a new number publishes`. Exported with the tenant from bundle format 5.0.0. **Not promoted:** a version is published and nothing more — no environments, no change set, no approval ([G-135](gaps.md)) |
...
| A score node's model pin resolved against the registry, and budgeted | BUILT | NO | NO | [ADR-009](adr/ADR-009-the-model-plane.md) §4–5. `modelPinProblems` in `packages/compiler/src/decision-flow/compile.ts` refuses a pin to a model the registry does not hold, a version never published, a kind a score node cannot read, a declared p95 that alone exceeds the budget, and a feature the data model does not define or types differently; the declared p95 joins the critical path. `packages/compiler/tests/compile.test.ts` › `model pins against the registry`. Only where the caller supplies the models: the console's development API does; the corpus builders and the Kotlin engine do not compile. **No console flow pins a model**, so no screen shows one of these refusals today |
| `Model` as a user-editable entity | BUILT | YES — `/models` | YES | `packages/ui-metadata/src/registry/model.ts`, rendered by the list–detail pattern from `packages/ui-metadata/src/layouts/models.ts`. Features are a `features` field — paths picked from the data model, each carrying the data model's type, so a declaration cannot disagree with it: `apps/console/tests/unit/features-field.test.tsx` › `takes the type from the data model, never from the person`. "Edit" publishes under the version the form names, because a version cannot change |
...
| A schema change reaches every database, once | BUILT | NO | NO | `packages/core/src/migrate.ts`: numbered files per store, each applied once in a transaction with a row recording its version and checksum. Refuses an edited, renamed or missing applied file, a gap, a file with its own transaction, and a database built before versioning. Checks `the runner, against a real database` and `reading a migration directory` (`packages/core/tests/migrate.test.ts`), each seen to fail with its refusal removed; `brings a database at every earlier version to the schema a fresh one has` in each store's `tests/migration.test.ts`; `migrations already on main` (`tests/migrations-frozen.test.ts`) refuses a pull request that edits or deletes a file already on main. [G-077](gaps.md) |
| A service never migrates on start | BUILT | NO | NO | [ADR-016](adr/ADR-016-deployment-operations-and-scale.md) §3.1. `verifyMigrations` in `packages/core/src/migrate.ts` reads a store's migration record and changes nothing — no lock, not even the version table — refusing a database behind the code (`BEHIND`, naming what it has not run) and allowing one ahead. All four store factories (catalogue, governance, ledger, registry) verify when `METIS_MIGRATIONS=verify` or, unset, in a production build; elsewhere they apply as before. Checks: `refuses a database behind the code, names what it has not run, and creates nothing` (`packages/core/tests/migrate.test.ts`); `refuses to open against a database the migration job has not run, and opens after it` (`planes/execution/tests/store-verify.test.ts`) |
| Migrations run as a one-shot job from the release image | BUILT | NO | NO | ADR-016 §3.2 and §3.5. `planes/execution/src/migrate-job.ts`, in the decision service image: every store in a fixed order, one line of the runner's output per store, exit 0 only when all are at the release's version, and at a refusal it stops and acts on none of the advice. `planes/execution/tests/migrate-job.test.ts` › `stops at a refusal, applies nothing after it, and does not act on its advice`; the `execution-image` CI job refuses the image against an empty database, runs the job twice from the image and requires the second run to change nothing. **Which environment runs it, and how its exit gates a rollout, is not built**: ADR-016 defers orchestration to the first customer's platform |
| A migration may add and never remove or tighten | BUILT | NO | NO | ADR-016 §3.3. `tests/migrations-expand-only.test.ts` refuses, in a migration the branch adds, a DROP, a RENAME, SET NOT NULL, a type change, a constraint added to an existing table, a NOT NULL column without a default, and DELETE, UPDATE or TRUNCATE. It reads the SQL, not its effect, so a statement it does not recognise passes |
| A configured database that cannot be reached | BUILT | NO | NO | An error, never a silent fallback to storage that forgets |
| Retention and erasure | PARTIAL — [W-006](BACKLOG.md) | NO | NO | Taxonomy 2.7, 2.12 `TABLE-STAKES`. [ADR-004](adr/ADR-004-retention-and-erasure.md) decides crypto-shredding per subject (accepted 2026-09-09); [ADR-025](adr/ADR-025-the-key-store.md) decides the key store (accepted 2026-09-19). `ENGINE-ONLY` in the survey's terms: **the key store is built and nothing is kept under it yet.** `packages/keys`: per-tenant keys held by a provider (a file, refused for a `real` tenant; a KMS, refused until a client is configured), per-subject keys stored wrapped, AES-256-GCM bound to the row, and the lifecycle — a key created on first write and never by a read, cached for at most 60 seconds, destroyed by erasure with an append-only record written first, and re-destroyed after a restore. One behaviour suite over memory and PostgreSQL (`packages/keys/tests/suite.ts`: `destroys the key, records it, and leaves what was sealed under it unopenable`, `reaches another process within the cache bound, and not before`, `is re-applied after a restore brings a destroyed key back`, `cannot be changed once recorded`), and `refuses a ciphertext moved to another row`. **Not built:** the ledger is still plaintext under an unkeyed subject hash (step B of `docs/DIRECTIVE.md`, [G-068](gaps.md)); no operation or screen can request an erasure; retention has no policy or job; and erasure takes up to 60 seconds to reach every process (ADR-025 §3) |
| Catalogue, policies and taxonomy in PostgreSQL | BUILT | NO | NO | `packages/catalogue` is durable — one behaviour suite over memory and a real database, foreign keys, a unique offer key, and an append-only edit log enforced by trigger. The console reads and writes it: with `METIS_DATABASE_URL` set, authored catalogue state survives a restart and the next decision is made against it (`apps/console/tests/unit/console-durable.test.ts` › `keeps an edit made through the API across a restart, and decides the next request against it`). Without a database the console still runs in memory. Corrected 2026-09-15: this row said the console had not been repointed, which the row below and W-005 contradicted. [W-005](BACKLOG.md) |
| Any store reachable from a screen | BUILT | YES — the authoring screens, `/approvals`, `/audit` | NO | `apps/console/mocks/store.ts` opens its catalogue, registry, governance store and ledger through `createCatalogueStore`, `createRegistryStore`, `createGovernanceStore` and `createLedgerStore`, so `METIS_DATABASE_URL` puts what a person does on a screen in PostgreSQL. Tests `keeps an edit made through the API across a restart, and decides the next request against it`, `keeps a flow drawn, published and promoted through the API across a restart, and the next decision runs it` and `keeps an approval across a restart: still approved, not approvable again, and still in the log` (`apps/console/tests/unit/console-durable.test.ts`). Data sources, tenant settings, autonomy settings and users are still in memory ([G-118](gaps.md)) |
...
| An offer cannot go active with nothing to deliver **on a channel somebody serves** | BUILT | YES — `/offers/[id]`, `/creatives?view=coverage` | NO | Taxonomy 4.12 `DIFFERENTIATING`. [W-057](BACKLOG.md), [ADR-012](adr/ADR-012-an-offer-with-nothing-to-render.md) option B. `domain.ts` said "at least one is required to go active" and enforced it nowhere; the guard that replaced it was `creatives.some((c) => c.active)`, which one active email creative satisfied for an offer about to win a web slot. Both `offerMayBeActive` and the compiler's `NO_DELIVERABLE_CREATIVE` now take the channels the tenant's active placements deliver on, and the compiler separates three states because the remedies differ: nothing written, everything switched off, and live content on channels this flow does not serve. `permissions-and-writes.spec.ts`, `packages/compiler/tests/compile.test.ts`; verified to bite by disabling each guard |
| Seeing which offers cannot be delivered, by channel | BUILT | YES — `/creatives?view=coverage` | NO | Taxonomy 5.7 `TABLE-STAKES`. One row per active offer against one column per channel the tenant can **deliver** on, in three states. **127** of 202 active offers have nothing live on a deliverable channel; it read 38 until 2026-09-10, when the denominator counted every *decidable* channel including four with nothing that sends ([ADR-013](adr/ADR-013-delivery.md) §2). Built as a view on `/creatives` rather than a route of its own: the same dataset asked a second question, with the answer in the URL so it can be linked. A Cascade (`METIS_CONSOLE_SPEC.md` §4.7) since 2026-09-14: active offers → written for a channel that delivers → switched on → every delivering channel, each a subset of the one above, the break drawn where an active offer has nothing live to send, and a selected stage showing exactly the offers that fell out there. The four filter blocks it replaced were a partition, not stages. `apps/console/lib/coverage.ts`; `apps/console/tests/unit/coverage.test.ts`, test `accounts for every active offer: what fell out at each stage, and what reached the last`; `apps/console/tests/e2e/creative-coverage.spec.ts` is `@screen-only`, test `the stages nest, and a stage shows exactly the offers that fell out there` |
...
| **Export / re-import** | **BUILT** | NO | NO | Taxonomy 15.14 `DIFFERENTIATING`. `packages/portability`. A populated tenant exports, imports into an empty instance and re-exports byte-identically; version hashes, ledger records and environment state including a running shadow and what it has compared all survive, and since bundle format 2.0.0 so do connectors and placements; and a pre-export decision replayed on the imported instance produces the same chain hash. `npm run export -- --tenant <id> --out <dir>`. **CLI only — no screen exports or installs anything** |
...
| Export covers approvals and the audit log | BUILT | NO | NO | `packages/portability/src/entities.ts` declares `governance_change_sets` and `governance_audit_events`, imported through the governance store so a decided change set stays decided and each event keeps its id and its place in the log. Test `keeps change sets as decided, and the audit log in the order it was written` (`packages/portability/tests/round-trip.test.ts`) |
...
| Route bundle budgets | BUILT | NO | NO | `npm run test:bundle` › `apps/console/tests/bundle/check-budgets.ts`: every route of the production build, read from the build's manifests (`route-chunks.ts`) — nine with budgets of their own, the rest held to a default. One build gives one number. `apps/console/tests/unit/route-chunks.test.ts` › `does not count a screen a route links to, so making that screen heavier moves nothing`. **Does not see** code a route loads at runtime that no manifest names; `apps/console/scripts/measure-route-payload.mjs` measures in a browser |
...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| Immutable audit log | BUILT | YES — `/audit` | NO | Taxonomy 15.7 `TABLE-STAKES`. Every write lands in it; `/audit` is filterable by actor type. Tests `the log does not observe itself`, `does not record reads of itself` — reads do not record, which is why the log stays legible. Since 2026-09-14 it is `@metis/governance`: append-only by trigger and kept across a restart. Tests `the database refuses to rewrite the audit log` (`packages/governance/tests/postgres.test.ts`) and `keeps an approval across a restart: still approved, not approvable again, and still in the log`. It is one of three logs, and `/audit` shows only this one ([G-120](gaps.md)) |
| Every route refuses what its nav entry declares | BUILT | YES — every route | NO | Taxonomy 15.2 `TABLE-STAKES`. [W-061](BACKLOG.md), [G-050](gaps.md). `RequireAuth` checked for a session and never for a permission: of twenty-one routes four hand-wrote a guard and sixteen enforced nothing, three of them while *declaring* a permission the rail obeyed and the route ignored, so the link was hidden and the URL was open. Enforcement is now derived in `apps/console/lib/nav/route-permissions.ts` from the same manifest `buildNav` reads and applied once in `apps/console/components/require-auth.tsx`, so a route cannot opt out by omission. A detail page inherits its list's permission — `/offers` was shut while `/offers/prop_5g_unlimited_24` was open, and offer ids are printed in traces. `tests/unit/route-authorisation.test.ts` (8) and `tests/e2e/route-authorisation.spec.ts` (7, `@screen-only`); verified to bite by reverting `RequireAuth`, by unwrapping a page, and by `/placements`, which was red on arrival |
| A gated permission that no account lacks | BUILT | NO — a check | NO | Taxonomy 15.2 `TABLE-STAKES`. The second reason the above survived a year: Sarah, Priya and Marcus held `view:offers`, `view:flows`, `view:decisions` and `view:audit` between them with no gaps, so no seeded account could be refused anything and no test could tell an enforced permission from an unenforced one. `route-authorisation.test.ts`, *"has an account refused each gated permission, one by one"*, fails a permission every fixture account holds **and** one no account holds. It caught its own author: Oliver Reed was created holding `view:decisions`, which all four accounts had |
...
| Change sets and approvals | BUILT | YES — `/approvals`, `/approvals/[id]` | NO | Taxonomy 15.1, 15.2 `TABLE-STAKES`. Agent vs person provenance, diff applied on approval. `writes persist`. A change set is decided once — by the store's conditional write and a trigger — so an approval cannot apply its diff twice, including after a restart. Tests `refuses to decide a change set twice, and keeps the first decision` and `has the store write a decision only over a pending change set` (`packages/governance/tests/suite.ts`). The decision and its diff are separate writes ([G-119](gaps.md)) |
...
| Consent as a decision input, enforced with a service exemption, failing closed when absent | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 2.2 `TABLE-STAKES`. `packages/runtime/src/deterministic/consent.ts` and `engines/kotlin/engine/src/main/kotlin/com/metis/engine/Consent.kt` carry marketing, profiling and third-party as purposes, each `granted`, `withheld` or `absent`. Tests `suppresses everything when marketing consent is withheld`, `absent consent is enforced as withheld and recorded as absent` (`packages/runtime/tests/consent.test.ts`), and the decision corpus cases of the same names, held in both engines. Withheld or absent marketing consent removes commercial offers and **not** duty-of-care ones. Since 2026-09-13 a request that states nothing is not granted, and the trace says `absent` rather than claiming consent (G-065). Since the same day it is applied to every decision, not only at a constraint node — next row (G-015). **The limit:** consent arrives on the request. Nothing reads it from the consent platform, stores it, ages it, or proves what it said at that instant — ADR-014 §7.2–§7.5 |
| Consent applied to every decision, whatever nodes the flow declares | BUILT | NO — no seeded decision reaches the platform step; the trace rail names it *Consent* (`apps/console/components/trace-cascade.ts`, test `shows the platform’s consent step as a stage, though no node declared it`) | NO | Taxonomy 2.3 `DIFFERENTIATING`. `packages/runtime/src/deterministic/engine.ts` and `engines/kotlin/engine/src/main/kotlin/com/metis/engine/Engine.kt`: a constraint node before ranking applies consent as it always did; otherwise the engine applies it before ranking and records step `__consent`. Tests `a flow with no constraint node has consent applied by the platform, before ranking` in `packages/runtime/tests/consent.test.ts` and `engines/kotlin/engine/src/test/kotlin/com/metis/engine/DecisionConformanceTest.kt`, and three decision corpus cases held in both engines. Until 2026-09-13 such a flow decided as though consent were granted; sent without consent, 20 of the corpus's 25 flows with no constraint node before ranking lose their winner ([G-015](gaps.md)). Frequency caps are still a node's to apply, and the compiler warns `ARBITRATION_WITHOUT_CONSTRAINT` when a flow has none — `packages/compiler/tests/compile.test.ts` |
...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| Shadow mode — a version beside the active one, deciding nothing | BUILT | YES — `/decision-flows/[id]` | NO | Taxonomy 13.10 `DIFFERENTIATING`. `packages/runtime/src/shadow`, `e2e/shadow.spec.ts` — nine tests. Verified to bite: pointing the shadow at the active version makes everything agree and the divergence assertion fails. Since 2026-09-14 what a shadow finds is kept in `registry_shadow_comparisons`, append-only, beside the shadow pointer, and survives a restart. Tests `keeps what a shadow found across a restart, and the report still counts it` (`apps/console/tests/unit/console-durable.test.ts`), `keeps shadow comparisons, oldest first, and narrows them to the pair asked about` (`packages/registry/tests/suite.ts`) |
...
| Complete export / re-import | **Met for everything durably stored.** The round-trip conformance utility passes on the catalogue, the registry with its drafts, change sets and the audit log, and the ledger. Data sources, tenant settings, autonomy settings and users are still in memory, so they are outside the export until they are outside memory — [W-005](BACKLOG.md), [G-118](gaps.md) |
...
| **Provable exitability** | **BUILT, and bounded.** A tenant round-trips byte-identically and a decision replayed after the move produces the same chain hash. Bounded because it covers what is persisted, which today is the catalogue, the registry with its drafts, change sets and the audit log, and the ledger | NO | NO |
...
| A condition reads the offer it judges, and compares it with another field | BUILT | YES — `/targeting-policies`, `/data-model` | NO | Taxonomy 6.3 `DIFFERENTIATING`. [ADR-017](adr/ADR-017-conditions-read-the-candidate.md), [G-075](gaps.md). A path beginning `offer.` reads the candidate from the catalogue snapshot, and a value may be `{ path }`: `policyPasses` in `packages/runtime/src/deterministic/engine.ts` and `engines/kotlin/engine/src/main/kotlin/com/metis/engine/Engine.kt`; `conditionProblems` and `schemaProblems` in `packages/core/src/profile-schema.ts`; the conditions field in `apps/console/components/ui/conditions-field.tsx`. Checks: decision corpus cases `a condition compares each candidate with the customer, and splits the decision`, `a condition reads a field of the candidate against a literal` and `a path value whose other side is missing fails closed` in both engines (`decision-conformance.test.ts`, `DecisionConformanceTest`); `candidate-conditions.test.ts`; `conditions-field.test.tsx`; `@screen-only a rule compares each offer with the customer, and keeps the comparison`. No arithmetic: "costs less than the current bill" is expressible, "costs at least $5 less" is not |
...
| The targeting policy form is declared, not hand-built | BUILT | YES — `/targeting-policies` | **YES** | Taxonomy 16.1 `FRONTIER`. `packages/ui-metadata/src/registry/targeting-policy.ts`, drawn by the generic renderer; its conditions are the `conditions` field type (ADR-006, amended 2026-09-13), which the server made necessary by refusing a policy with none. Tests `every descriptor matches its OpenAPI schema`, `the TargetingPolicy descriptor, exercised`, `authoring a policy`. Scope is still not asked: every policy authored here is tenant-wide |
| A targeting policy, a placement and a creative can be deleted from the screen | PARTIAL | YES — `/targeting-policies`, `/placements` | NO | [G-110](gaps.md). `deleteTargetingPolicy`, `deletePlacement` and `deleteCreative` are proposed operations served by the console's development API only, each refusing while something depends on the record. Tests `deleting a targeting policy`, `deleting a placement`, `deleting a creative` (`apps/console/tests/unit/delete-api.test.ts`), `deletes a policy it wrote, and it leaves the list`, `a slot configured by mistake can be deleted, and leaves the registry` |
| The ranking formula's weights are declared, not hand-built | BUILT | YES — `/arbitration` | **YES** | Taxonomy 16.1 `FRONTIER`. `packages/ui-metadata/src/registry/arbitration-config.ts`, drawn inline by the generic renderer as sliders (`presentation: 'slider'`), beside the formula and the ranking they preview. Tests `the ArbitrationConfig descriptor, exercised`, `draws every weight as a slider, so the ranking can be moved under the hand`, `a weight moved on the screen changes decisions only once its change set is approved`. Edited only: a tenant has one, and one without it has no ranking function |
| Move a weight and the ranking reorders, with movement and ties marked, and nothing publishes until a change set is raised | BUILT | YES — `/arbitration` | NO | The fibre-address scenario runs down the decision path without being recorded (`getArbitrationScenario`), and `packages/core/src/arbitration.ts` ranks its terms under the weights on screen with the engine's ranking function, rounding and key tie-break. A tie is two offers sharing a priority after rounding, found from the numbers. Raising calls `createChangeSet`, served since 2026-09-14, and the weights change on approval. Tests `dropping the boost weight reorders the ranking, warns of the tie, and raises a change set that publishes nothing` (`apps/console/tests/e2e/arbitration-live.spec.ts`), `agrees with the engine on every priority and the winner, across a grid of weights`, `finds the tie in the data when the boost weight is zero, and none at the live weights` (`apps/console/tests/unit/arbitration-agreement.test.ts`), `opens a pending change set, publishes nothing, and records who raised it` (`apps/console/tests/unit/change-set-create.test.ts`). The weights API still publishes without a change set, and only arbitration weights can be raised ([G-122](gaps.md)) |
...
| A profile schema as a first-class object — two roots, typed fields declaring origin and class, declared aggregations | PARTIAL | YES — `/data-model` | NO | Taxonomy 1.9, 1.20 `TABLE-STAKES` / `DIFFERENTIATING`. [ADR-014](adr/ADR-014-the-data-spine.md) §2, accepted 2026-09-11. `packages/core/src/profile-schema.ts`; tests `resolving a path`, `the schema itself`. `Customer` and `Context` replace the single `DecisionInput` root, and every field declares `origin` — `profile`, `request`, `interaction`, `aggregation` or `connector:<id>` — and `class`. **The limits:** the screen is read-only, so a computed attribute can be seen and not defined; `class` is declared and acted on by nothing, because consent is §7 and its own slice; and the schema is not yet a registry object published through change sets |
| A decision names the data model it was read against | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 1.21 in part. [ADR-014](adr/ADR-014-the-data-spine.md) §2, [G-069](gaps.md), [W-074](BACKLOG.md). The compiler hashes the schema content into `{ id, version, hash }` and the artifact pins it — `packages/compiler/src/decision-flow/compile.ts`, test `what compilation works out about a node`; `DeterministicDecision.schema` carries the same triple, `null` where an artifact pins none, and both engines hash it identically — `docs/conformance/decision-corpus.json` case `a pinned schema is part of the decision`, checked by `packages/runtime/tests/decision-conformance.test.ts` and `engines/kotlin` `:engine:test`. **The limit:** a live flow that does not compile has no compiled artifact and therefore no pin, and one of this tenant's three live flows is in that state — [G-071](gaps.md) |
| A rule reads a value an integration supplied, and the trace names the integration | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 1.15, 14.4. [G-069](gaps.md). `pol_credit_pass` reads `customer.credit_band` from `conn_credit_bureau`; connectors declare where a value lands as a path into the profile and `resolveInputs` writes it there — `packages/runtime/src/integration/resolve.ts`, test `POST /api/decisions resolves its connectors`. Until 2026-09-11 policies read dotted paths and connectors declared flat names, so no rule read a connector-supplied value and the seeded decisions consumed no integration data at all |
...
| A cap counts the platform's own contacts, not only the caller's | PARTIAL | YES — `/decisions/[id]` | NO | Taxonomy 12.1 `TABLE-STAKES`. [ADR-021](adr/ADR-021-frequency-caps-count-the-ledger.md). The console's placement decision reads each customer's contacts per channel from the ledger — decisions that offered something and were dispatched or delivered, rolling 24h, 7d and 30d back from `occurredAt` — adds them to the caller's `contactHistory`, and records the read on the decision as `contactsRead`; an unreadable ledger suppresses what a cap covers with `CONTACT_HISTORY_UNAVAILABLE`. Tests `a cap counts the platform’s own contacts` (`apps/console/tests/unit/caps-read-ledger.test.ts`), `contacts, as a cap counts them` (`packages/ledger/tests/suite.ts`, memory and PostgreSQL), `contacts the platform read` (`packages/runtime/tests/contacts-read.test.ts`); both engines held by three decision-corpus cases. A cap scoped to an objective, category or offer counts only the contacts about its scope, recorded as `contactsRead.scoped` and refused by both engines when missing (ADR-021 §9); the corpus case *"a cap scoped to an offer counts the contacts about that offer, not the channel’s"*, `ScopedCapTest`, and the scoped test in `caps-read-ledger.test.ts`. No fixture cap is scoped until ADR-019's reseed. PARTIAL because `POST /api/decisions` and the decision service still count only what the caller sends ([G-150](gaps.md)) |
...
| A simulation that does not touch live decisions or the interaction log | ABSENT | NO | NO | Taxonomy 13.5 `TABLE-STAKES`. Both simulation operations, `simulateDecisionFlow` and `getCounterfactual`, are `x-metis-status: proposed` in `docs/metis-api.openapi.yaml`, and nothing serves either: `apps/console/app/api/[...path]/route.ts` has no handler for them. `/simulations` lists change sets and shows the simulation results written into `apps/console/mocks/fixtures/governance.ts`, and says ad-hoc simulation is specified but not implemented (`apps/console/app/simulations/page.tsx`). Corrected 2026-09-15: this row was BUILT and cited `the two gaps close`, a test in `apps/console/tests/unit/flow-authoring.test.ts` about a created offer becoming decidable, not about simulation. [ADR-018](adr/ADR-018-the-seeded-corpus-becomes-ledger-rows.md) names why a bias ratio cannot be computed yet |
...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| The loop read as one decomposition, break included | BUILT | YES — `/`, `/performance` | NO | Taxonomy 14.2 `TABLE-STAKES`. [W-060](BACKLOG.md), [G-049](gaps.md). `docs/METIS_CONSOLE_SPEC.md` §4.7 Cascade, and the first screen on it: decisions → offered → deliverable → seen → acted, five stages that nest. The break at `deliverable` — offered decisions won on a channel nothing sends — is drawn in the block colour with its count stated on the stage, and every rate below it names the channel it describes. The Overview draws the same loop since 2026-09-13, under the proposed changes and agent activity, from the same model: `apps/console/lib/loop.ts`, which also refuses a stage larger than the one above it, and `apps/console/components/loop-panes.tsx`. `apps/console/components/cascade-rail.tsx`, `apps/console/components/loop-flow.tsx`; `apps/console/tests/e2e/performance-cascade.spec.ts` and `apps/console/tests/e2e/overview.spec.ts` are `@screen-only`; `apps/console/tests/unit/loop.test.ts`, test `refuses a stage larger than the one above, which means two populations`. At low volume since 2026-09-17: a stage that cannot drop — Deliverable, when every channel that offered something delivers it — is drawn as a pass-through that says why, and the flow diagram draws five columns at every volume, with no bar for a stage at zero; `apps/console/tests/unit/loop-pass-through.test.tsx` ("what the funnel shows"), and `expectFunnelShows` in `overview.spec.ts` and `loop-by-hand.spec.ts` on the rendered Overview. Realised value names the decisions it rests on (`valued` on the report), says it is thin below 100 of them with a spread computed from the count, and carries the page's accent only at or above that floor ([ADR-023](adr/ADR-023-realised-value-states-what-it-rests-on.md)); `apps/console/tests/unit/realised-value-floor.test.tsx`. The Overview draws it on two panes from 2026-09-17 — rail and stage, no evidence column, six cards on a row, the cause of the drop on the stage that dropped — and `overview.spec.ts` holds it to fitting a 1680×1000 viewport without scrolling; `/performance` keeps the third pane and the fuller shape |
...
| Outcomes produced, joined to their decision, and reported — for web | BUILT | YES — `/storefront/index.html`, `/performance` | NO | Taxonomy 11.8 `TABLE-STAKES` in part. [ADR-008](adr/ADR-008-closing-the-outcome-loop.md) phase one. The storefront reports `impression` when a slot renders an offer, `click` when someone clicks it, and a click then an `acceptance` carrying the panel's value when someone presses **Accept** on a single-slot placement (since 2026-09-17), bound by decision id and nothing else. `tests/e2e-empty/loop-by-hand.spec.ts` is `@screen-only`: Accept on an empty tenant reaches realised value on `/performance`. `outcome-loop.spec.ts` is `@screen-only`: it opens the storefront as a visitor, clicks as a customer, then opens `/performance` as a marketer and reads a real rate. **The limits, all deliberate:** web only — nothing is sent, queued or bounced, so this does not prove outbound and [W-017](BACKLOG.md) stays open; no acceptance on the grid, whose outcomes cannot name the card ([ADR-020](adr/ADR-020-a-slate-is-recorded-as-shown.md) §4), and the acceptance's value is a placeholder typed by the person, because offers carry no price ([G-089](gaps.md)); and the decision behind the rate cannot be opened, which is a defect in the trace route registered in [`gaps.md`](gaps.md) rather than a property of the loop |
| A seeded outcome requires a creative **and** a deliverer | BUILT | YES — `/performance` | NO | Taxonomy 14.22 `FRONTIER`. [W-059](BACKLOG.md), [G-046](gaps.md). The corpus recorded 887 impressions against 738 deliverable decisions — an inverted funnel, and 471 impressions of messages nothing could send. Now 416 seen, 79 acted. `apps/console/tests/unit/seeded-outcomes.test.ts`, *"starts no funnel where nothing delivers the winning channel"*; verified to bite by removing the rule |
| The report refusing to state what it cannot know | BUILT | YES — `/performance` | NO | Taxonomy 14.22 `FRONTIER`. `performance.ts:11` and the closing paragraph of `apps/console/app/performance/page.tsx` both say it in the product's own words: *"Counting only: no attribution or uplift."*. Tests `what the numbers refuse to say`, `absent is not zero` enforce the refusal, so the honesty cannot quietly lapse |
...
| The seeded corpus reports its own outcomes | BUILT | YES — `/performance` | NO | Taxonomy 14.1, 14.12 `TABLE-STAKES`. [ADR-008](adr/ADR-008-closing-the-outcome-loop.md) phase two. `apps/console/mocks/fixtures/synthetic-customers.ts` derives impressions, clicks, acceptances, rejections and conversions from the same seed as the decisions, and since [ADR-018](adr/ADR-018-the-seeded-corpus-becomes-ledger-rows.md) §5 the seed job writes them through `recordOutcome`: **1,561 events across 1,159 decisions** since the reseed of 2026-09-19 (1,654 across 1,228 before it: its stage 0 re-keyed the outcome draws, and its stage 4 capped Disney+ on the web), which are rows a screen can open rather than a projection computed at read time — `tests/unit/seeded-ledger.test.ts`, `writes 1,561 events across 1,159 decisions, by type`. `/performance` reads **1,159 seen of 1,647 deliverable**, and the screen says the other 488 have no outcome recorded rather than reading them as refusals. It read 2,101 of 3,425 until [G-041](gaps.md), [G-046](gaps.md) and [G-049](gaps.md) took the numerator down twice and gave the denominator a stage of its own. `tests/unit/seeded-outcomes.test.ts` holds the shape over all 10,400 — the funnel nests on every decision, coverage stays between 45% and 80%, web reports more than an outbound call, value appears on conversions and nowhere else, and the churn cohort converts materially worse |
...
| Authored catalogue survives a restart — taxonomy, offers, creatives, policies, boosts, the ranking function, connectors, placements, the profile schema and experiments | BUILT | YES — the screens that author them | NO | `apps/console/mocks/store.ts` reads and writes through `@metis/catalogue`, opened by `apps/console/mocks/catalogue-source.ts`: an empty store is seeded, one holding the tenant is used as found, one holding only another tenant is refused. `METIS_DATABASE_URL` puts it in PostgreSQL. Tests `keeps an edit made through the API across a restart, and decides the next request against it`, `uses a database that already holds the tenant as it finds it, never reseeding over an edit` and `refuses a database that holds another tenant and not this one, and writes nothing` (`apps/console/tests/unit/console-durable.test.ts`). Data sources, tenant settings, autonomy settings and users do not survive yet ([G-118](gaps.md)); a live decision made before a restart cannot be replayed after it ([G-116](gaps.md)) |
| Decision flows survive a restart — drafts, published versions, environments and the registry's event log | BUILT | YES — `/decision-flows`, `/decision-flows/[id]` | NO | `apps/console/mocks/store.ts` opens `@metis/registry` through `apps/console/mocks/registry-source.ts`: an empty registry is seeded by publishing the fixture flows against the catalogue as stored, one holding the tenant's drafts is used as found, one holding published flows and no drafts is refused. Tests `keeps a flow drawn, published and promoted through the API across a restart, and the next decision runs it` (`apps/console/tests/unit/console-durable.test.ts`) and `judges the seed against the catalogue as stored, not the fixtures` (`apps/console/tests/unit/registry-source.test.ts`). Some readings of a flow still come from the fixtures ([G-121](gaps.md)) |
| Change sets and the audit log survive a restart | BUILT | YES — `/approvals`, `/audit` | NO | `apps/console/mocks/store.ts` opens `@metis/governance` through `apps/console/mocks/governance-source.ts`, with the catalogue's seeding rule. Tests `keeps an approval across a restart: still approved, not approvable again, and still in the log` (`apps/console/tests/unit/console-durable.test.ts`) and `gives an empty store the fixture change sets as decided, and the log in its order` (`apps/console/tests/unit/governance-source.test.ts`). An approval and its diff are separate writes ([G-119](gaps.md)) |
| A synthetic number says so, wherever it goes | BUILT | YES — `/performance`, `/decisions`, `/decisions/[id]` | NO | Taxonomy 14.3 `FRONTIER` in part. The seeded tenant became indistinguishable from real reporting on 2026-09-09 — 10,400 decisions, hundreds of measured outcomes, plausible rates, realised value in pounds — and the only marker was an untested badge in the nav rail, which a screenshot of the report does not include. Now: a `Provenance` schema in the spec, carried on the decision search, the trace, the outcomes list and the performance report; a banner rendered **above the figures in the content column** on all three screens; and the marker inside both export payloads as the first key. `tests/unit/provenance.test.ts` holds all three routes out of the building — API, export, screenshot — including that the banner is on each named screen and is **not** in the app shell, because that is where it failed before |
...
| The canvas carrying traffic — each edge as thick as the candidates that crossed it | PARTIAL | YES — `/decision-flows/[id]` | NO | `docs/METIS_CONSOLE_SPEC.md` §4.2. A toggle any reader can use, each node showing what it removed, and a legend saying what thickness means. Counted from the eliminations each decision recorded, over the flow's graph in the engine's visit order: `packages/ledger/src/flow-volume.ts`, served as `getFlowVolume`, which is **proposed** and answered by the console's development API, not a plane ([G-127](gaps.md)). Drawn with the geometry the loop diagram uses, so a thickness is the same arithmetic on both screens: `apps/console/components/volume/geometry.ts`, `apps/console/components/canvas/volume-edge.tsx`. `apps/console/tests/e2e/flow-volume.spec.ts` is `@screen-only`; `apps/console/tests/unit/flow-volume-route.test.ts`, test `is a decomposition: every node’s arrivals are its removals and survivors, and every edge carries its source’s survivors`; `apps/console/tests/unit/volume-drawing.test.tsx`, test `scales every edge over the graph’s largest crossing, and gives an edge the report does not name nothing`. **Not live:** the window is the last 24 hours ending at the newest decision recorded, not the clock. **Absent:** clicking a node to see who took that path, which the spec asks for beside it |
...
| A deployment that holds real customer data refuses to write it where the subject is stored in clear | BUILT | NO | NO | ADR-016 §4, ADR-004's amendment, [G-068](gaps.md). `packages/ledger/src/create-store.ts`: `SUBJECT_PROTECTION` is `none`, a constant of the code, and `createLedgerStore` refuses before connecting when `METIS_DATA_CLASS` is `real` and a database is configured. The decision service refuses to start without a declared data class (`planes/execution/src/config.ts`) and reports it on `/health`. Tests `refuses real data in postgres while the subject is stored in clear — ADR-004, G-068`, `the decision service configuration`; CI job `execution-image` requires the built image to refuse both. Protection stays `none` until ADR-004 points 1–3 exist, so there is no `real` deployment yet |
...
| 10,400 decisions over 24 months, with seasonality and a churn cohort | BUILT | YES — `/decisions` | NO | Every decision is a real execution of `@metis/runtime` over the seeded catalogue. Volume ramps toward the present and peaks before Christmas; the hour-of-day peak is 18:00 and the trough 03:00. One customer in eleven is in a churn cohort, so their decisions suppress on `CONSENT_WITHHELD` rather than being labelled. `apps/console/tests/unit/seeded-ledger.test.ts`, `apps/console/tests/unit/seeded-outcomes.test.ts` |
| The corpus is executed once at start rather than on every import | BUILT | NO | NO | It was a committed index of flat rows (2.2 MB) until [ADR-018](adr/ADR-018-the-seeded-corpus-becomes-ledger-rows.md) §6 deleted it. `apps/console/mocks/store.ts` seeds the ledger once per process when `METIS_SEED_LEDGER` is set — 10,400 decisions, 10,400 deliveries and 1,561 outcomes (1,654 before the reseed of 2026-09-19), about 3.2s on a CI runner — and logs what it wrote; a reset restores the same history without executing again. The cost is bounded by a check rather than by a habit: `apps/console/tests/e2e/warmup.setup.ts` fails the run if the seed exceeds 45 seconds |
...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| A landing page per persona, chosen by a switch for an account that can see both | BUILT | YES — `/` | NO | The marketer lands on the loop and the decision architect on the change pipeline, as panels; an account holding both, or an administrator, switches in the chrome and the choice is remembered per person in the browser; one with neither persona gets the loop and no switch. `apps/console/lib/persona.ts`; `apps/console/tests/unit/persona.test.ts` (7), pinning the seeded tenant's four accounts; `apps/console/tests/e2e/overview.spec.ts` `@screen-only`, `an account that is both switches, and the choice survives a reload` |
...
| Eight of sixteen user-editable entities actually declared | PARTIAL | YES — `/objectives`, `/placements`, `/offers`, `/targeting-policies`, `/arbitration` | NO | `registry/index.ts` holds `Objective`, `Category`, `Placement`, `Offer`, `Creative`, `TenantSettings`, `TargetingPolicy` and `ArbitrationConfig`, and a read-only `ChangeSet` that no screen writes; `USER_EDITABLE_ENTITIES` lists sixteen; `PENDING` records why each of the other eight is absent, per entity, with a reason. Rule 8 is enforced for **50%** of the entities it names, up from 33% on 2026-09-13 and 14% on 2026-09-09. A test fails if an entity is in neither list, so the mechanism tracking the debt is better built than the mechanism it tracks |
| A tenant's locale and currency, which every date, number and amount on every screen is formatted in | BUILT | YES — `/settings` | **YES** | [G-092](gaps.md). `TenantSettings` on `/tenants/{tenantId}/settings` (`getTenantSettings`, `updateTenantSettings`); the form is the `TenantSettings` descriptor, drawn by the generic renderer, and changing locale or currency needs no code. `apps/console/lib/format.ts` is the only file that calls the platform formatters, and `useFormat()` reads the tenant's. Tests `switching the tenant locale changes dates and money on every screen` (`tests/e2e/tenant-locale.spec.ts`, `@screen-only`), `calls the platform formatters nowhere but lib/format.ts` (`tests/unit/locale-formatting.test.ts`), `refuses a locale the runtime cannot format, naming the field` (`tests/unit/tenant-settings-api.test.ts`), `refuses to guess a currency when neither the record nor the tenant gives one` (`packages/ui-metadata/tests/descriptors.test.ts`). Dates are still in the viewer's time zone — [G-100](gaps.md) |
...
| Route bundle size was in the definition of done and checked by nothing | **Closed** — `npm run test:bundle` reads what every route loads from the production build's manifests (a browser measurement until 2026-09-19). Verified: a route 0.1 kB over budget fails |
