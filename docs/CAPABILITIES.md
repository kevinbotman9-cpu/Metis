...
| Stable reason codes, per candidate | BUILT | YES — `/decisions/[id]` | NO | Taxonomy 14.4 `DIFFERENTIATING`. 9 codes in `deterministic/types.ts:229-260`; `decision-conformance.test.ts` asserts the corpus exercises **every** one and `trace-cascade.test.ts` asserts the screen explains every one, both reading the closed set rather than a hand-written list. `NOT_ACTIVE`, `OUT_OF_VALIDITY_WINDOW`, `ELIGIBILITY_FAILED`, `RELEVANCE_FAILED`, `SUITABILITY_FAILED`, `CONSENT_WITHHELD`, `FREQUENCY_CAP_BREACHED`, `COOLDOWN_ACTIVE`, `NOT_RANKED` |
