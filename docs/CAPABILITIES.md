...
| The engine cannot reach the network at all outside a configured connector | BUILT | NO | NO | Taxonomy 17.12 `FRONTIER`. `packages/runtime/tests/no-egress.test.ts` › `Network blocked` — `fetch` throws `Network blocked` inside the decision path. The taxonomy classes this as frontier; it is here and tested |
...
| The trace declaring the propensity is not a model | BUILT | YES — `/decisions/[id]` | NO | `packages/runtime/src/deterministic/engine.ts` › `not a trained model (W-029)` — the trace reads *"a pinned deterministic function, not a trained model (W-029)"*. The comment above it records why: the sentence used to read like a real model had scored, and nobody had written a false claim — a pinned model id made one anyway |
...
| `Model` as a user-editable entity | ABSENT | NO | NO | `packages/ui-metadata/src/registry/index.ts` › `Gate 2. No schema, no screen.`: *"Gate 2. No schema, no screen."* |
...
**One reference is misfiled.** `packages/runtime/src/deterministic/engine.ts` › `not a trained model (W-029)` and
`apps/console/mocks/fixtures/artifacts.ts` › `W-029` @ `b79a1fa1b` both point a reader to W-029.
...
| Bulk import of a catalogue with per-row failures | PARTIAL | NO | NO | Taxonomy 4.8 `TABLE-STAKES`. `packages/portability/src/import.ts` round-trips a whole tenant, and `packages/core/src/intake.ts` › `validateRows` validates rows and reports problems per row for data sources. Neither is a catalogue import a person can start from a screen |
...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| The three qualification tiers as separately named policy kinds | BUILT | YES — `/targeting-policies` | NO | Taxonomy 6.1, 6.2, 6.3 `TABLE-STAKES` / `DIFFERENTIATING`. `packages/core/src/domain.ts` › `export type PolicyKind` — `PolicyKind = 'eligibility' \| 'relevance' \| 'suitability'`. Each has its own denial reason in the trace. The distinction is modelled rather than collapsed into one filter |
...
| Move a weight and the engine reranks the scenario, with movement marked, and nothing publishes until a change set is raised | PARTIAL | YES — `/arbitration` | NO | Every priority on the screen is the engine's: `previewArbitration` runs the fibre-address scenario under the live and the proposed weights, records nothing, and returns both rankings in `rankByPriority` order. It is a proposed operation served by the console's development API ([G-123](gaps.md)). Weights that move no offer are found from the scores and labelled on their sliders — propensity and context in this tenant ([G-124](gaps.md)). Raising calls `createChangeSet`, and the weights change on approval. Tests `ranks at the live weights exactly as a recorded decision at that slot, and records nothing itself`, `ranks proposed weights as the engine does under them, across a grid`, `leaves the proposed weights out of the catalogue registry`, `names propensity and context as moving no offer in this tenant, and not value or boost` (`apps/console/tests/unit/arbitration-preview.test.ts`), `moving boost reorders the ranking, the inert weights say they move nothing, and raising publishes nothing` (`apps/console/tests/e2e/arbitration-live.spec.ts`). The weights API still publishes without a change set ([G-122](gaps.md)); the design this came from is a draft ([G-125](gaps.md)) |
| Rules validated against the profile schema at authoring time | BUILT | YES — `/targeting-policies` | NO | Taxonomy 1.11 `TABLE-STAKES`. `packages/core/src/profile-schema.ts` › `operatorsFor`, `didYouMean`; operators are constrained by field type and a bad path gets a spelling suggestion. Test `policy conditions against the data model` |
...
| Aggregations over history resolved at decision time | BUILT | YES — `/data-model` | NO | `packages/runtime/src/integration/aggregate.ts`, `packages/runtime/src/integration/resolve.ts` › `resolveInputs`; tests `usage becomes decision input`, `merging into the input` |
| Data source intake: define, map, transform, validate, activate | BUILT | YES — `/data-model/intake` | NO | Taxonomy 1.1, 1.13 `TABLE-STAKES`. `packages/core/src/intake.ts` › `mapRow`, `applyTransform`, `validateRows`; tests `mapping a row`, `transforms`, `the validation report`, `activation`. Per-row problems are reported, not a single rejection |
| A frequency & suppression policy with cap, period and scope, enforced at a constraint node | BUILT | YES — `/frequency-policy` | NO | Taxonomy 12.1, 12.2, 12.3 `TABLE-STAKES`. `packages/core/src/domain.ts` › `FrequencyPolicyPeriod`, `export interface FrequencyPolicy {`, `packages/runtime/src/deterministic/engine.ts` › `node.type === 'constraint'`; tests `what a cap must be`, `precedence and determinism`. A scope-specific cap can raise the global one, and the trace names the first breached cap in catalogue order |
| Changing the frequency & suppression policy | PARTIAL | YES — `/frequency-policy` | NO | Taxonomy 12.1 `TABLE-STAKES`. `packages/ui-metadata/src/registry/index.ts` › `Read-only screen today; no write endpoint is served.`: *"Read-only screen today; no write endpoint is served."* The engine enforces a policy nobody can change without editing a fixture |
...
| Volume constraints — a finite inventory with per-period usage | PARTIAL | NO | NO | Taxonomy 4.7 `DIFFERENTIATING`. `packages/core/src/volume.ts` › `VolumeConstraint`, `resolveVolume`; tests `what a cap must be`, `resolveVolume`. **No screen and no route reads or writes a constraint.** Survey verdict ENGINE-ONLY |
| Flow test cases run against a catalogue snapshot | PARTIAL | NO | NO | Taxonomy 13.8 `FRONTIER`. `packages/runtime/src/flow-tests/index.ts` › `FlowTestExpectation`; test `running a flow author`. **No route runs these** — a tester cannot reach them from a screen. Survey verdict ENGINE-ONLY |
...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| Experiments with arms, shares and a holdout | BUILT | YES — `/experiments` | NO | Taxonomy 9.1 `TABLE-STAKES`. `packages/core/src/experiment.ts` › `ExperimentArm`; tests `what an experiment must be`, `creates a holdout as a draft, assigning nobody yet` |
| Assignment as a pure function, recorded nowhere | BUILT | YES — `/experiments` | NO | Taxonomy 9.20 `TABLE-STAKES` — inverted. `packages/core/src/experiment.ts` › `bucketOf`; test `assignment is a function, not a record`. Deterministic bucketing means no assignment table can drift, **and** means there is no assignment record to export for independent analysis |
| A started split frozen against edits, with the refusal explained | BUILT | YES — `/experiments` | NO | Taxonomy 9.3 `DIFFERENTIATING`. `packages/core/src/experiment.ts` › `editProblems`; tests `a running experiment is frozen`, `explains why a started split cannot be changed` |
| An arm reachable from a targeting policy at a declared field path | BUILT | YES — `/experiments`, `/targeting-policies` | NO | Taxonomy 9.2 `DIFFERENTIATING`. `packages/core/src/experiment.ts` › `armPath`; tests `arms as decision input`, `refuses a key that would collide at the same field path`. This is the join that makes an experiment change a decision rather than only a message |
| The arm recorded as part of what was decided | BUILT | YES — `/decisions/[id]` | NO | Test `an arm is part of what was decided` |
| Performance broken down by arm | BUILT | YES — `/experiments` | NO | `packages/ledger/src/performance.ts` › `buildPerformance`; test `performance by arm`. Counting only |
...
| A performance report over real decision records | BUILT | YES — `/performance` | NO | Taxonomy 14.1 `TABLE-STAKES`. `packages/ledger/src/performance.ts` › `PerformanceRow`, `buildPerformance`; tests `report`, `the report reaches real outcomes`, `performance by arm`. Built from the corpus at request time, not a precomputed table |
...
| Five channels modelled end to end in the catalogue and the trace | BUILT | YES — `/creatives` | NO | Taxonomy 11.2 `TABLE-STAKES`. `packages/core/src/domain.ts` › `export type Channel` — email, SMS, web, push, outbound call. Modelled, not delivered |
...
| Layout manifests — screens declared rather than coded | ABSENT | NO | NO | Taxonomy 16.1 `FRONTIER`. `packages/ui-metadata/src/registry/index.ts` › `Layout manifests do not exist yet` @ `b79a1fa1b`: *"Layout manifests do not exist yet."* This is the largest block of the standing conformance failures: `/`, `/performance`, `/settings`, `/simulations`, `/targeting-policies` and others each fail `[layout-manifests]` |
...
| Capability | Status | Screen? | Config? | Evidence |
|---|---|---|---|---|
| Build an audience in a visual editor and see the count before saving | ABSENT | NO | NO | Taxonomy 3.1 `TABLE-STAKES`. `packages/ui-metadata/src/registry/index.ts` › `No schema in the spec and no screen`: *"No schema in the spec and no screen"* |
...
path name at `apps/console/components/nav-rail.tsx` › `journeys:` and a doc comment at
`packages/core/src/domain.ts` › `A content slot in a customer journey`. The decision flow canvas is a DAG executed to
...
| `W-029` is cited from source and registered in the wrong file | Open — `packages/runtime/src/deterministic/engine.ts` › `not a trained model (W-029)` and `apps/console/mocks/fixtures/artifacts.ts` › `W-029` @ `b79a1fa1b` point at `gaps.md`; W-029 lives in `docs/BACKLOG.md` › `### W-029` |
