...
|---|---|---|
| Overview | Cascade (4.7) for the marketer; Dashboard (4.5) for the decision architect | Two landing pages over one tenant, chosen by persona (`apps/console/lib/persona.ts`). An account that can see both — a marketer who is also an architect, or an administrator — chooses with a switch in the chrome, remembered per person; one with neither persona gets the loop and no switch. **The marketer's** is the loop — decisions → offered → deliverable → seen → acted — as rail, selected stage and evidence, with the proposed changes and agent activity above it. Changed from *Dashboard* by the product owner on 2026-09-13: the loop has a spine, and the dashboard it replaced put the product's thesis below four doughnuts. **The architect's** is the change pipeline as panels — proposed, simulated, released, flows, and what the engines are held to — each with its window and a link to the screen that owns it. A *Dashboard*, not a Cascade: those counts are of different things, and a rail over them would claim a subset relationship that is not there (§4.7). The product owner's own mockup drew it as a rail and withdrew that on 2026-09-13. Both hand-built until their renderers exist (ADR-015). |
