...
|---|---|---|---|---|---|
| A | Erasable | **The key store**: decided by ADR — store, cipher, lifecycle, what ADR-004 left open — then built: a per-tenant key provider, per-subject keys, erasure with a record and a check | — | **Chosen** to go first: nothing forces it ahead of C or F, but B, D and G cannot start without it, so it unblocks the most | [ADR-025](adr/ADR-025-the-key-store.md) accepted 2026-09-19, with §5's Version A. **Built 2026-09-19** as `packages/keys`: nothing is held under it until step B |
