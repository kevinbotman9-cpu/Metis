...
CREATE TABLE IF NOT EXISTS registry_versions (
    ...
    -- strategy that shipped near its latency budget is a different thing to
    -- explain in six months than one that shipped clean.
    warnings       jsonb       NOT NULL DEFAULT '[]'::jsonb,

    PRIMARY KEY (tenant_id, strategy_name, version)
);

CREATE INDEX IF NOT EXISTS registry_versions_by_strategy
    ON registry_versions (tenant_id, strategy_name, published_at DESC);
...
CREATE TABLE IF NOT EXISTS registry_environments (
    ...
    PRIMARY KEY (tenant_id, strategy_name, environment),
    ...
    FOREIGN KEY (tenant_id, strategy_name, active_version)
        REFERENCES registry_versions (tenant_id, strategy_name, version)
        DEFERRABLE INITIALLY DEFERRED,
    FOREIGN KEY (tenant_id, strategy_name, previous_version)
        REFERENCES registry_versions (tenant_id, strategy_name, version)
        DEFERRABLE INITIALLY DEFERRED
);
...
CREATE INDEX IF NOT EXISTS registry_events_by_strategy
    ON registry_events (tenant_id, strategy_name, seq DESC);
