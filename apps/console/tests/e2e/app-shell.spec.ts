...
test.describe('header band', () => {
  ...
  test('states the environment and the tenant permanently', async ({ page }) => {
    ...
    await expect(page.getByText('Only one tenant is provisioned in this environment.', { exact: true })).toBeVisible();

    await page.keyboard.press('Escape');
    await expect(page.getByText('Only one tenant is provisioned in this environment.', { exact: true })).toBeHidden();
  });
