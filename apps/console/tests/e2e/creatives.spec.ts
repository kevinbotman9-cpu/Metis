...
test.describe('the content library', () => {
  ...
  test('lists content across every offer, not one offer at a time', async ({ page }) => {
    ...
    await expect(page.getByText('20 of 20 creatives')).toBeVisible();

    // Two different offers on the page, so "across offers" is checked rather
    // than assumed.
    await expect(page.getByRole('link', { name: '5G Unlimited 24mo' }).first()).toBeVisible();
    await expect(page.getByRole('link', { name: 'Full Fibre 900Mb' }).first()).toBeVisible();
  });
  ...
  test('shows the line each channel actually leads with', async ({ page }) => {
    ...
    // a from-address, which reads as content and is not.
  ...
  test('narrows to a channel', async ({ page }) => {
    ...
    await expect(page.getByText('homepage_grid')).toBeVisible();
