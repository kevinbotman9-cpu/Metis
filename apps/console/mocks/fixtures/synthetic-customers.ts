...
/**
 * The synthetic-customer model — ADR-018 §5.1.
 *
 * Moved here unchanged from `outcomes.ts` in slice 2a. It is the one place the
 * seeded tenant's behaviour is invented: which decisions produce an impression,
 * a click, an acceptance, a rejection or a conversion, when, and for how much.
 * The seed job applies it once, gated on the delivery each decision's own
 * placement recorded (`outcomesFor`). The projection applies it gated on the
 * channel (`seededOutcomesFor`) until slice 3 deletes that read path, and
 * `tests/unit/seeded-ledger.test.ts` holds the two equal decision by decision.
 */
...
/** Whether any placement on this channel delivers: the projection's gate, until slice 3. */
export function deliversOnChannel(channel: string): boolean {
...
export function outcomesFor(d: DecisionRecord, gate: { dispatched: boolean }): OutcomeEvent[] {
  ...
  //
  // Which gate says so is the caller's: the seed job passes what the decision's
