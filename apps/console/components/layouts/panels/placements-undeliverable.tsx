 ...
 * of a channel with no recipient address, and every decision made for one
...
export function PlacementsUndeliverable({ rows }: PanelProps) {
  ...
  return (
    <div className="rounded border border-hold/40 bg-hold-subtle px-cell py-cell-y">
      ...
      <p className="mt-1.5 text-label text-content-muted">{undeliverable.map((p) => String(p.name)).join(', ')}</p>
    </div>
  );
}
